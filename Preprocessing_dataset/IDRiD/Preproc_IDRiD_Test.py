...
# Iterate over each image in the CSV file
for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing IDRiD Test Set", ncols=100, ascii=True):
    image_name_no_ext = row[column_image_name]
    image_name = image_name_no_ext + file_extension
    image_class = str(row[column_class_name])
    src_path = os.path.join(image_dir, image_name_no_ext + file_extension)
    output_class_path = os.path.join(final_output_dir, image_class)

    # Check for .jpeg extension if .jpg is not found
    ...
    if img_bgr is None:
        print(f"Error reading image at {src_path}")
        error_count += 1
        continue
    ...
    try:
        ...
        # 2. Segment the fundus, create a mask, and get the bounding box
        fundus_mask, fundus_bbox = segment_fundus_and_create_mask(img_cv2_gray.copy(), image_name)

        # 3. Apply CLAHE, Gaussian Blur, and Median Filter
        ...
        if fundus_bbox and fundus_bbox[2] > 0 and fundus_bbox[3] > 0:
            ...
            # Convert to a PIL image for letterboxing
            image_to_letterbox_pil = cv2_to_pil_grayscale(resized_cropped_fundus_cv2)
        else:
            # Handle cases with an invalid or no bounding box
            if fundus_bbox:
                print(f"Warning: Invalid fundus bounding box for {image_name}. Resulting image will be black.")
            else:
                print(f"Warning: No fundus bounding box found for {image_name}. Resulting image will be black.")
            image_to_letterbox_pil = Image.new('L', (1, 1), 0) # Create a placeholder image
            null_bbox_count += 1
            
        # 5. Letterbox: Pad the image to the final size (512x512)
        letterboxed_img_pil = letterbox_image(image_to_letterbox_pil, FINAL_IMAGE_SIZE, padding_color=0)
        
        # 6. Convert back to a CV2 NumPy array for saving
        final_img_to_save = pil_to_cv2_grayscale(letterboxed_img_pil)
        
        # 7. Save the processed image as a PNG file
...
# --- Final Report ---
print("\n--- Processing Report ---")
print(f"Number of images with reading/processing errors: {error_count}")
print(f"Number of images with a null or invalid fundus bounding box: {null_bbox_count}")
